...
for data_index, data in enumerate(data_list):
    ...
    index_list = []
    slowness_index = []
    temp_data = finaltemp_array_list[temp_index]
    swe_data = finalSWE_array_list[swe_index]
    ...
    axes[1].set_ylabel('Relative Power',fontsize=7)
    axes[2].plot(time,temp_data[3,:],'b',linewidth=3) # Plots temperature
    axes[2].set_ylabel('Temperature (Fahrenheit)',fontsize=7)
    axes[3].plot(time,swe_data[3,:],'g',linewidth=3) # Plots SWE
    axes[3].set_ylabel('Snow Water Equivalent (inches)',fontsize=7)
    plt.savefig('Figures/'+'24hr_subplots/' + dates[date_index] + '_24hrs.jpg',dpi=200)
    plt.clf()
    plt.close()
